...
* [`src/kinto_todo/`](src/kinto_todo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/kinto_todo/core.cljs): contains the SPA entry point, `init`
* [`test/kinto_todo/`](test/kinto_todo/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `kinto-todo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/kinto_todo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns kinto-todo.example
  (:require [kinto-todo.config :as config])
